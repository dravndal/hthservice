...
if (isset($_SESSION['brukernavn'])) {
    ...
    if (count($users) > 0) { // Hvis det er minst 1 verdi i arrayet
        ...
        $userTable .= "<tr><th>Fornavn</th><th>Etternavn</th><th>Kode</th><th>Epost</th><th>Slett</th></tr>"; // Append table headers
        /* For hver bruker, lag en table row med data om brukeren */
        foreach ($users as $user) {
            $userTable .= "<tr><td>".$user['fornavn']."</td><td>".$user['etternavn']."</td><td>".$user['kode']."</td><td>" .$user['epost']."</td><td><button class='delete-button' onclick='deleteUser(this.value)' value='".$user['kode']."'>Slett</button></td></tr>";
        }
